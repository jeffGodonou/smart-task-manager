...
- Build tool: Maven
- Persistence: JPA / Hibernate · H2 (in-memory for development)
...
The `TaskService` class implements task lifecycle operations, enforces user ownership checks, and delegates persistence responsibilities to the repository layer.
...
On Windows, run the wrapper with `mvnw.cmd` instead of `./mvnw`.
